...
example : ∃ (x y z : nat), pythag_triple x y z  :=
begin
apply exists.intro 3,
apply exists.intro 4,
apply exists.intro 5,
...
-- Enlish proof (it's short!): n + (m + k) = (n + k) + m by the associative property of addition
...
--multiple_of (n m : ℕ) := ∃ (k), n = m * k
example : ∃ (n : nat), multiple_of n 6 → multiple_of n 3:=
begin
apply exists.intro 18,
unfold multiple_of,
assume h,
cases h with w pf,
...
--18 mo 6 ∧ 6 mo 3 → 18 mo 3
example (n h k : ℕ) : multiple_of n h 
→  multiple_of h k → multiple_of n k :=
begin
unfold multiple_of,
assume nh,
assume hk,
...
example 
  ...
  (∃ (p), isCool p) :=
begin
cases SomeoneKnowsLogic with w pw,
apply exists.intro w,
--have h:=LogicalMakesCool Ben
--have BenIsCool := h BenKnowsLogic
--exact BenIsCool
apply LogicMakesCool w pw,
end
...
assume h,
assume allhappy,
cases h with p nothappy,
have person := allhappy p,
contradiction,
end
...
assume l,
assume p,
cases (classical.em(P p)) with left right,
exact left,
let false := l (exists.intro p right),
contradiction,
end 
...
assume h,
assume t,
cases (classical.em (P t)) with l r,
let tl := exists.intro t l,
let false := h (exists.intro t l),
contradiction,
exact r,
end
...
example 
  ...
  (P : α → Prop)
  (Q : α → Prop): 
  (∃ p : α, P p ∨ Q p) → (∃ p : α, P p) ∨ (∃ p : α, Q p) :=
begin
assume h,
cases h with p pf,
cases pf with pp qp,
apply or.inl,
exact exists.intro p pp,
apply or.inr,
exact exists.intro p qp,
end
